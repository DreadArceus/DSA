class LinkCutTree
{
    typedef struct _Node
    {
        ...
        _Node(int y) : val(y), cnt(1), rev(false), self(0), sum(0), l(nullptr), r(nullptr), p(nullptr), pp(nullptr)
        {
        }
        ~_Node()
        {
    ...
    Node access(Node v)
    {
        ...
        while (v->pp)
        {
            ...
            w->splay();
            if (w->r)
                w->r->p = nullptr, w->r->pp = w;
            v->pp = nullptr, w->r = v, v->p = w, w->calc();
